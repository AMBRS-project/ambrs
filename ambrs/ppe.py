...
PPE are sampled"""
    name: str
    modal_size: AerosolModalSizeDistribution
    ...
    if spec.modal_size:
        modal_size = AerosolModalSizePopulation(
            modes=tuple([
                ...
                ) for mode in spec.modal_size.modes]),
        )
    return Ensemble(
        specification = spec,
...
def lhs(spec: EnsembleSpecification,
        n: int,
        criterion = None,
...
distribution from which ensemble members are sampled."""
    ...
    if spec.modal_size:
        ...
        num_species = len(spec.modal_size.species)
        modal_size = AerosolModalSizePopulation(
            modes=tuple([
                AerosolModePopulation(
                ...
                ) for m, mode in enumerate(spec.modal_size.modes)]),
        )
    return Ensemble(
        specification = spec,
        modal_size = modal_size,
