...
def retrieve_model_state(
    ...
    partmc_population_cfg = {
        ...
        'repeat':repeat_num, # number of PartMC repeat, if run multiple times
        'species_modifications':species_modifications}
    # Debug: print the partmc_dir and list its contents to help debugging path issues
    try:
        logging = __import__('logging')
        logging.getLogger(__name__).debug('retrieve_model_state: partmc_dir=%s', partmc_dir)
    except Exception:
        print('retrieve_model_state: partmc_dir=', partmc_dir)
    try:
        print('partmc_dir exists:', os.path.isdir(partmc_dir))
        print('partmc_dir listing:', os.listdir(partmc_dir) if os.path.isdir(partmc_dir) else [])
        out_dir = os.path.join(partmc_dir, 'out')
        print('out_dir exists:', os.path.isdir(out_dir))
        print('out_dir listing:', os.listdir(out_dir) if os.path.isdir(out_dir) else [])
    except Exception as e:
        print('Error listing directories for debugging:', e)
...
#         n_repeat = self.n_repeat
#         if n_repeat>1:
#             warn('Analysis of PartMC ensembles not implemented; assuming n_repeat = 1')
...
#         ''' 
#         for tt in len(self.get_timesteps(output_dir, prefix, ensemble_number=ensemble_number)):
...
# timestep and ensemble index, given the directory in which it resides"""
#         output_dir = os.path.join(dir, 'out')
#         full_prefix = self.get_full_prefix(output_dir, prefix, ensemble_number=ensemble_number)
#         ncfiles = self.get_ncfiles(output_dir, prefix, ensemble_number=ensemble_number)
#         if len(ncfiles) == 0:
...
#     def get_full_prefix(self, output_dir, prefix, ensemble_number=1):
#         full_prefix = prefix + '_' + str(int(ensemble_number)).zfill(4)
#         return full_prefix
    
#     def get_ncfiles(self, output_dir, prefix, ensemble_number=1):
#         full_prefix = self.get_full_prefix(dir, prefix, ensemble_number=1)
#         ncfiles = [f for f in os.listdir(output_dir) if f.startswith(full_prefix) and f.endswith('.nc')]
#         return ncfiles
        
#     def get_timesteps(self, output_dir, prefix, ensemble_number=1):
#         ncfiles = self.get_ncfiles(output_dir, prefix, ensmble_number=ensemble_number)
#         N_timesteps = len(ncfiles)
#         return N_timesteps
