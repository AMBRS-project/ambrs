...
working directory contains any needed input files."""
        return f'{exe}'
    # FIXME: LMF correcting mixtakes; double-check.
    def write_input_files(self, input, dir, prefix) -> None:
        ...
        if hasattr(self, "camp") and self.camp:
            # CAMP species present in Scenario gases (optional hint; CAMP ignores extras)
            gases = [g.name for g in getattr(input, "gases_present", [])] if hasattr(input, "gases_present") else None
            camp_files_abs = self.camp.write_for_model(dir, model_name="mam4", gases_present=gases)
        # ---------------------------------
